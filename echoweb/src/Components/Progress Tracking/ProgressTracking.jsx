...
const ProgressTracking = () => {
  ...
  return (
    ...
    <button class={`btn  mx-2 rounded-pill my-1 ${activecourseButton === 'course' ? 'btn-dark ' : 'btn-outline-dark '} `} onClick={() => handlecourseButtonClick('course')}  style={{height:'48px', width:'150px'}}> Course</button>

    <button class={`btn  mx-2 rounded-pill my-1 ${activecourseButton === 'course' ? 'btn-outline-dark' : 'btn-dark '} `} onClick={() => handlecourseButtonClick('workout')} style={{height:'48px', width:'150px'}}>Workout Sessions</button>

    </div>
    <div className='d-flex justify-content-center container'>
      <button
        type="button"
        className={`w-100 border-0 toggle-button ${activeButton === 'ongoing' ? 'progressactive' : 'border-bottom'}`}
      ...
      <button
        type="button"
        className={`w-100 border-0 toggle-button ${activeButton === 'completed' ? 'progressactive' : 'border-bottom'}`}
        style={{ backgroundColor: 'white' }}
    ...
    {activeButton === 'ongoing' && <ProgressTrackingWorkoutOngoing/>}
      {activeButton === 'completed' && <ProgressTrackingWorkoutCompleted/>}
    </div>
    </>}
    </div>
    </>
